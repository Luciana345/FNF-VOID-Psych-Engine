...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		bg = new FlxSprite(0, 1300).loadGraphic(Paths.image('titleBG'));
		bg.updateHitbox();
		add(bg);
		...
		switch(easterEgg.toUpperCase())
		{
			...
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
				gfDance.frames = Paths.getSparrowAtlas('titleVoid' + FlxG.random.int(1, 5));
				gfDance.animation.addByPrefix('bop', 'void bumpin', 24, false);
		}
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			{
				FlxTween.tween(bg,{y: 0}, 2.8, {ease: FlxEase.expoOut});
