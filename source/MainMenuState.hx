...
class MainMenuState extends MusicBeatState
{
	...
	var easterEggKeys:Array<String> = ['STAR'];
	var allowedKeys:String = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
	var easterEggKeysBuffer:String = '';

	var connectedToGamejolt:Bool = false;
	override function create()
	{
		if (!connectedToGamejolt)
		{
			GameJoltAPI.connect();
			GameJoltAPI.authDaUser(FlxG.save.data.gjUser, FlxG.save.data.gjToken);
			connectedToGamejolt = true;
		}
	...
	function password(name:String)
	{
		...
		for (wordRaw in easterEggKeys)
		{
			...
			if (easterEggKeysBuffer.contains(word))
			{
				...
				black.setGraphicSize(Std.int(black.width * 1.175), Std.int(black.height * 1.175));
				black.alpha = 0;
				add(black);
