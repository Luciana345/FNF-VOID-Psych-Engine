...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (FlxG.save.data.secretUnlocked)
			addSong('Stardust', 0, 'star', FlxColor.fromRGB(146, 113, 113));
	...
	override function update(elapsed:Float)
	{
		...
		{
			if (songs[curSelected].songName.toLowerCase() == 'stardust')
			{
				var achieveID:Int = Achievements.getAchievementIndex('star_unlock');
				if(!Achievements.isAchievementUnlocked(Achievements.achievementsStuff[achieveID][2])) {
					Achievements.achievementsMap.set(Achievements.achievementsStuff[achieveID][2], true);
					Achievements.giveAchievement('star_unlock', function() {
						persistentUpdate = false;
